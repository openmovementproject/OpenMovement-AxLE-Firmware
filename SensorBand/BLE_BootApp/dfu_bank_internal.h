...
#include "ble_types.h"
#include "boards.h"
// KL: Added to allow timeout control via config and variable
#include "Config.h"
...
// KL: Changed to allow external state visibility to determine if the DFU is active (difference between "just connected" and "connected to an actual update server")
extern dfu_state_t	m_dfu_state;				/**< Current DFU state. (m_dfu_state > DFU_STATE_RDY) */
// KL: Timeout: Now depends on connection state.
typedef enum {DFU_TIMEOUT_INITIAL, DFU_TIMEOUT_EXTENDED, DFU_TIMEOUT_OCCURRED} DfuTimeoutState_t;
// KL: To determine connection status
...
// KL: Added prototype to allow external timeout restart (for disconnect case)
extern uint32_t dfu_timer_restart(void);

// #define APP_TIMER_PRESCALER		 0											   /**< Value of the RTC1 PRESCALER register. */

//#define DFU_TIMEOUT_INTERVAL_INITIAL	APP_TIMER_TICKS(10000, APP_TIMER_PRESCALER)		/**< DFU initial timeout interval in units of timer ticks. Short period, wait for update server */	 
//#define DFU_TIMEOUT_INTERVAL_INACTIVE		APP_TIMER_TICKS(120000, APP_TIMER_PRESCALER)	/**< DFU timeout interval when active in units of timer ticks. Update server connected, DFU active */	 
