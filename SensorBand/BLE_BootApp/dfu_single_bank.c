...
dfu_state_t				  m_dfu_state;				/**< Current DFU state. */
 ...
 *		   image.
 */
uint32_t dfu_timer_restart(void)
{
	if (m_dfu_timed_out)
	{
	...
	// Restart timeout using current setting. KL: Added restart() on dis/connect also to reduce time out wait.
	err_code = app_timer_start(m_dfu_timer_id, m_dfu_timeout, NULL);
    APP_ERROR_CHECK(err_code);
...
uint32_t dfu_init(void)
{
	...
	// KL: Start the DFU timer using global timeout setting
	err_code = app_timer_start(m_dfu_timer_id, m_dfu_timeout, NULL);
	APP_ERROR_CHECK(err_code);
