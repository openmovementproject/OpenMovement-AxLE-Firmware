...
#include "dfu_bank_internal.h" // KL: Added for timer reset call
...
// KL: LED indication moved to main() part of DFU application
...
// KL: Moved to use as a setting...#define APP_ADV_INTERVAL					 MSEC_TO_UNITS(25, UNIT_0_625_MS)						/**< The advertising interval (25 ms.). */
...
// KL: Removed bulky App->DFU bond share path.#define APP_DIRECTED_ADV_TIMEOUT			 50													   /**< number of direct advertisement (each lasting 1.28seconds). */
// KL: Removed bulky App->DFU bond share path. #define PEER_ADDRESS_TYPE_INVALID			0xFF													/**< Value indicating that no valid peer address exists. This will be the case when a private resolvable address is used in which case there is no address available but instead an IRK is present. */   
...
// KL: Defined in dfu_bank_internal.h
...
static const ble_gap_sec_params_t m_sec_params = 															/**< Security requirements for this application. */
{ // KL: Made constant for reduced code size
	.bond		 = SEC_PARAM_BOND,
	.mitm		 = SEC_PARAM_MITM,
	.lesc		 = SEC_PARAM_LESC,
...
// KL: Allow connection status detection externally
//static uint16_t			 m_conn_handle			= BLE_CONN_HANDLE_INVALID;					  /**< Handle of the current connection. */
uint16_t				m_conn_handle			= BLE_CONN_HANDLE_INVALID;					  /**< Handle of the current connection. */
static bool				 m_is_advertising		 = false;										/**< Variable to indicate if advertising is ongoing.*/
// KL: Removed bulky App->DFU bond share path.static dfu_ble_peer_data_t  m_ble_peer_data;														 /**< BLE Peer data exchanged from application on buttonless update mode. */
// KL: Removed bulky App->DFU bond share path.static bool				 m_ble_peer_data_valid	= false;										/**< True if BLE Peer data has been exchanged from application. */
// KL: Removed bulky App->DFU bond share path.static uint32_t			 m_direct_adv_cnt		 = APP_DIRECTED_ADV_TIMEOUT;					 /**< Counter of direct advertisements. */
 ...
 */
 // KL: Removed bulky App->DFU bond share path.
...
static void advertising_start(void)
{
	if (!m_is_advertising)
	{
		...
		// Set transmit power for advertising - low to limit range
		err_code = sd_ble_gap_tx_power_set(DFU_ADVERT_TX_POWER);
		APP_ERROR_CHECK(err_code);
...
//		else
		...
		// KL: Note: Since the ADV TX power value is null, it is not encoded into the advert
		err_code = sd_ble_gap_adv_start(&m_adv_params);
		APP_ERROR_CHECK(err_code);

		// KL: LED tasks moved to main
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
	...
	switch (p_ble_evt->header.evt_id)
	{
		case BLE_GAP_EVT_CONNECTED:
            ...
            // Set transmit power for connections (higher power for reliable transfer)
			err_code = sd_ble_gap_tx_power_set(DFU_CONNECTED_TX_POWER);
			APP_ERROR_CHECK(err_code);

			// KL: Change timeout if required and restart timeout
			if(m_dfu_timeout != DFU_TIMEOUT_INTERVAL_NO_APP)
				m_dfu_timeout = DFU_TIMEOUT_INTERVAL_ACTIVE;
			dfu_timer_restart();

			break;

		case BLE_GAP_EVT_DISCONNECTED:
			// Update connection vars
			{
				uint8_t  sys_attr[128];
				uint16_t sys_attr_len = 128;
			
				// KL: m_direct_adv_cnt = APP_DIRECTED_ADV_TIMEOUT;
				// KL: LED tasks moved to main
			...
			// KL: Change timeout if required and restart timeout
			if(m_dfu_timeout != DFU_TIMEOUT_INTERVAL_NO_APP)
				m_dfu_timeout = DFU_TIMEOUT_INTERVAL_INACTIVE;
			dfu_timer_restart();
...
//				keys.keys_peer.p_enc_key = &enc_key;
				...
				err_code = sd_ble_gap_sec_params_reply(m_conn_handle,
													   ...
													   (ble_gap_sec_params_t const *)&m_sec_params, /*KL: Cast const array address to pointer const */
													   NULL);// KL: Removing imported connection key function -&keys);
				APP_ERROR_CHECK(err_code);
			}
			break;
		...
		case BLE_GAP_EVT_TIMEOUT:
			// KL: REmoved unused directed advertising modes. Adverts are stopped by a timer now.
...
//				else
		...
		case BLE_GAP_EVT_SEC_INFO_REQUEST:
			{
				// KL: ble_gap_enc_info_t * p_enc_info = NULL;

// KL: Removed passing encryption data from App->DFU
...
//				}
				// KL: Clear all bond data for a fresh connection without bonding
				err_code = sd_ble_gap_sec_info_reply(p_ble_evt->evt.gap_evt.conn_handle,
													 NULL, // KL: p_enc_info,
													 NULL, // KL: &m_ble_peer_data.irk,
													 NULL);
				APP_ERROR_CHECK(err_code);
			}
			break;

// KL: Trying to remove un-necessary security module and save connection attributes
...
//					
		case BLE_GATTS_EVT_SYS_ATTR_MISSING:
            // No system attributes have been stored, bonding disabled
            err_code = sd_ble_gatts_sys_attr_set(m_conn_handle, NULL, 0, 0);
            APP_ERROR_CHECK(err_code);
            break;

		case BLE_GAP_EVT_CONN_SEC_UPDATE:
			// Secure connections are disabled, the secure mode never changes even if updated. Ignore event
//			err_code = service_change_indicate();
//			APP_ERROR_CHECK(err_code);
			break;
...
// KL: Removed this complex app/boot bonding sharing process due to excess code size
...
//	}
//	else
// KL: This next section makes the DFU address different from the App address by incrementing the LSB of the MAC
	{
		ble_gap_addr_t addr;
